...
def list_view(request):
    ...
    try:
        categories = category_service.get_all_ticket_categories(
            search_event_id=search_query if search_query else None)
    except DatabaseError as e:
        error_msg = str(e).split('\n')[0]
        if "invalid input syntax for type uuid" in error_msg:
            messages.error(
                request, "Format ID Event tidak valid (Harus UUID).")
        else:
...
def create_view(request):
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT event_id, event_title FROM EVENT ORDER BY event_title ASC;")
    ...
    if request.method == 'POST':
        ...
        result = category_service.create_ticket_category(
            session_id, name, quota, price, event_id)
        if result:
            return redirect('category_list')
        else:
            return render(request, 'ticket_category/category_form.html', {
                'error': 'Gagal menambah kategori. Penambahan kuota ini melebihi sisa kapasitas venue.',
                'role': role,
                'events': events,
                'dashboard': profile_data,
                'username': profile_data.get('username')
...
def update_view(request, category_id):
    ...
    if request.method == 'POST':
        ...
        # Panggil service (quota dulu, baru price)
        success = category_service.update_ticket_category(
            session_id, category_id, name, quota, price)
        if success:
            return redirect('category_list')
        else:
            return render(request, 'ticket_category/category_form.html', {
